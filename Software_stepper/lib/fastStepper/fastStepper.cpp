...
void fastStepper::init() {
  ...
  timerAttachInterrupt(_timer,timerFun);
}
...
void IRAM_ATTR fastStepper::timerFunction() {
  ...
  if (!pinState) {
    ...
    REG_WRITE(GPIO_OUT_W1TS_REG, 1 << _stepPin);
    pinState = 1;
  } else {
    REG_WRITE(GPIO_OUT_W1TC_REG, 1 << _stepPin);
    pinState = 0;
  }
...
void fastStepper::update() {
  ...
  if (lastSpeed!=speed) {
    if (absSpeed!=0) {
      ...
      timerAlarm(_timer, absSpeedInt, true, 0);
      if (!timerEnable) {
        timerStart(_timer); // Re-enable timer
        timerEnable = 1;
      }
    } else {
      timerStop(_timer);
