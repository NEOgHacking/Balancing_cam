...
# MQTT dashboard
This branch also supports MQTT-based robot status publishing and command control.

## Robot firmware MQTT behavior
- The ESP32 connects to a WiFi network configured in preferences.
- It publishes status updates every second to `robots/<robotName>/status`.
- Each status message is JSON and includes:
  - `name`
  - `battery`
  - `angle`
  - `state`
- The robot subscribes to:
  - `robots/<robotName>/command`
  - `robots/all/command`
- Supported commands:
  - `stop` — disable control and stop the robot
  - `selfright` / `restart` — trigger a self-right attempt

## Running the PC dashboard
From the `Software` folder, install the Python dependency and start the GUI:
...
The dashboard will:
- discover available robots via `robots/+/status`
- display a status card for each robot
- show battery voltage, angle, and control state
- provide buttons for `Stop` and `Restart` per robot
- provide a global `Stop All Robots` button

## Broker configuration
The default broker is set to `broker.hivemq.com` on port `1883`.

### Using a local broker
For a local MQTT broker, install Mosquitto or another MQTT broker on your network machine.
- On Linux: `sudo apt install mosquitto`
- On macOS: `brew install mosquitto`

Then update the connection target:
- in `src/main.cpp`, change `kMqttBroker` to your broker IP or hostname, for example `"192.168.1.42"`
- in `mqtt_dashboard.py`, either edit `BROKER`/`PORT` or set environment variables:
...
If the broker is running on the same PC as the dashboard, use `localhost` or `127.0.0.1`.

If the broker is on your local network, use the broker's LAN IP address so the ESP32 can reach it.
